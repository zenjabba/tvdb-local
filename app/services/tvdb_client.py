...
class TVDBClient:
    ...
    def _get_client(self) -> tvdb_v4_official.TVDB:
        ...
        if not self.client or not self._authenticated:
            try:
                ...
                logger.error("Failed to authenticate TVDB client", error=str(e))
                raise
        return self.client
    
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
    async def get_series(self, series_id: int, use_cache: bool = True) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            cached = self.cache.get_series(series_id)
            if cached:
                logger.warning("Returning stale cached series data", series_id=series_id)
                return cached
        
        return None
    
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
    async def get_series_extended(self, series_id: int, use_cache: bool = True) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if series_data:
                ...
                logger.debug("Extended series fetched from API", series_id=series_id)
                return series_data
                
        except Exception as e:
            logger.error("Failed to fetch extended series", series_id=series_id, error=str(e))
            cached = self.cache.get_series(cache_key)
            if cached:
                logger.warning("Returning stale cached extended series data", series_id=series_id)
                return cached
        
        return None
    
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
    async def get_series_episodes(self, series_id: int, page: int = 0, use_cache: bool = True) -> Optional[Dict[str, Any]]:
        ...
        if use_cache:
            ...
            if cached:
                logger.debug("Series episodes cache hit", series_id=series_id, page=page)
                return cached
        ...
        try:
            ...
            if episodes_data:
                self.cache.set("episodes", cache_key, episodes_data, settings.cache_ttl_dynamic_hours)
                logger.debug("Series episodes fetched from API", series_id=series_id, page=page)
                return episodes_data
                
        except Exception as e:
            logger.error("Failed to fetch series episodes", series_id=series_id, page=page, error=str(e))
            cached = self.cache.get("episodes", cache_key)
            if cached:
                logger.warning("Returning stale cached episodes data", series_id=series_id, page=page)
                return cached
        
        return None
    
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
    async def get_episode(self, episode_id: int, use_cache: bool = True) -> Optional[Dict[str, Any]]:
        """Get episode by ID"""
        if use_cache:
        ...
        try:
            ...
            logger.warning("Direct episode fetch not implemented in library", episode_id=episode_id)
            return None
                
        except Exception as e:
            logger.error("Failed to fetch episode", episode_id=episode_id, error=str(e))
            cached = self.cache.get_episode(episode_id)
            if cached:
                logger.warning("Returning stale cached episode data", episode_id=episode_id)
                return cached
        
        return None
    
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
    async def get_season_extended(self, season_id: int, use_cache: bool = True) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if season_data:
                self.cache.set("season", season_id, season_data, settings.cache_ttl_dynamic_hours)
            ...
            cached = self.cache.get("season", season_id)
            if cached:
                logger.warning("Returning stale cached season data", season_id=season_id)
                return cached
        
        return None
    
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
    async def get_movie(self, movie_id: int, use_cache: bool = True) -> Optional[Dict[str, Any]]:
        """Get movie by ID"""
        if use_cache:
        ...
        try:
            ...
            cached = self.cache.get_movie(movie_id)
            if cached:
                logger.warning("Returning stale cached movie data", movie_id=movie_id)
                return cached
        
        return None
    
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
    async def get_movie_extended(self, movie_id: int, use_cache: bool = True) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if movie_data:
                ...
                logger.debug("Extended movie fetched from API", movie_id=movie_id)
                return movie_data
                
        except Exception as e:
            logger.error("Failed to fetch extended movie", movie_id=movie_id, error=str(e))
            cached = self.cache.get_movie(cache_key)
            if cached:
                logger.warning("Returning stale cached extended movie data", movie_id=movie_id)
                return cached
        
        return None
    
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
    async def get_person_extended(self, person_id: int, use_cache: bool = True) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Failed to fetch person", person_id=person_id, error=str(e))
            cached = self.cache.get_person(person_id)
            if cached:
                logger.warning("Returning stale cached person data", person_id=person_id)
                return cached
        
        return None
    
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
    async def get_all_series(self, page: int = 0, use_cache: bool = True) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if series_data:
                # Cache for shorter time since this is a list that changes frequently
                self.cache.set("series_list", cache_key, series_data, 1)  # 1 hour
    ...
    async def search_series(self, query: str, use_cache: bool = True) -> Optional[List[Dict[str, Any]]]:
    ...
    async def invalidate_cache(self, entity_type: str, entity_id: int):
        ...
        logger.info("Cache invalidated", entity_type=entity_type, entity_id=entity_id)
