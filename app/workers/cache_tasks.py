...
from app.redis_client import cache, TVDBCache
from app.models import Series, Episode, Movie, Person
...
def cleanup_expired_cache(self):
    ...
    try:
        ...
        self.update_state(state='PROGRESS', meta={'current': 0, 'total': 100, 'status': 'Starting cache cleanup'})
        
        # Clean up expired search results (they have short TTL)
        cleaned_search = cache.flush_pattern("search:*")
        self.update_state(state='PROGRESS', meta={'current': 25, 'total': 100, 'status': 'Search cache cleaned'})
        
        # Clean up stale episode lists (regenerate from fresh data)
        cleaned_episodes = cache.flush_pattern("episodes:*_episodes_page_*")
        self.update_state(state='PROGRESS', meta={'current': 50, 'total': 100, 'status': 'Episode lists cleaned'})
        
        # Clean up temporary data
        cleaned_temp = cache.flush_pattern("temp:*")
        self.update_state(state='PROGRESS', meta={'current': 75, 'total': 100, 'status': 'Temporary data cleaned'})
        
        # Get cache statistics after cleanup
        stats_after = cache.get_cache_stats()
        
        self.update_state(state='PROGRESS', meta={'current': 100, 'total': 100, 'status': 'Cache cleanup completed'})
        ...
        return {
            "status": "completed",
            "cleaned_entries": cleaned_search + cleaned_episodes + cleaned_temp,
            "stats_before": stats_before,
            "stats_after": stats_after
        }
...
def prefetch_popular_content(self):
    ...
    try:
        with get_db_session() as db:
            self.update_state(state='PROGRESS', meta={'current': 0, 'total': 100, 'status': 'Finding popular content'})
            
            # Get popular series (by popularity score or recent access)
            popular_series = _get_popular_series(db, limit=50)
            self.update_state(state='PROGRESS', meta={'current': 20, 'total': 100, 'status': 'Prefetching popular series'})
            ...
            self.update_state(state='PROGRESS', meta={'current': 50, 'total': 100, 'status': 'Prefetching popular movies'})
            ...
            self.update_state(state='PROGRESS', meta={'current': 80, 'total': 100, 'status': 'Prefetching trending episodes'})
            ...
            self.update_state(state='PROGRESS', meta={'current': 100, 'total': 100, 'status': 'Prefetch completed'})
...
def warm_cache_for_series(self, series_id: int):
    ...
    try:
        ...
        logger.error("Cache warming failed for series", series_id=series_id, error=str(e))
        self.update_state(state='FAILURE', meta={'error': str(e)})
        raise
...
def rebuild_search_index(self):
    ...
    try:
        with get_db_session() as db:
            self.update_state(state='PROGRESS', meta={'current': 0, 'total': 100, 'status': 'Clearing old search index'})
            
            # Clear existing search cache
            cache.flush_pattern("search:*")
            
            # Build series search index
            self.update_state(state='PROGRESS', meta={'current': 20, 'total': 100, 'status': 'Building series search index'})
            _build_series_search_index(db)
            
            # Build movie search index
            self.update_state(state='PROGRESS', meta={'current': 60, 'total': 100, 'status': 'Building movie search index'})
            _build_movie_search_index(db)
            
            # Build people search index
            self.update_state(state='PROGRESS', meta={'current': 80, 'total': 100, 'status': 'Building people search index'})
            _build_people_search_index(db)
            
            self.update_state(state='PROGRESS', meta={'current': 100, 'total': 100, 'status': 'Search index rebuild completed'})
...
async def _prefetch_series_data(series_id: int):
    ...
    try:
        ...
        logger.warning("Failed to prefetch series data", series_id=series_id, error=str(e))
...
async def _prefetch_movie_data(movie_id: int):
    ...
    try:
        ...
        logger.warning("Failed to prefetch movie data", movie_id=movie_id, error=str(e))


async def _prefetch_episode_data(episode_id: int, series_id: int):
    """Prefetch episode data"""
    try:
        # Episodes are typically fetched via series, so prefetch series episodes
        await tvdb_client.get_series_episodes(series_id, use_cache=False)
        
        logger.debug("Episode data prefetched", episode_id=episode_id, series_id=series_id)
    except Exception as e:
        logger.warning("Failed to prefetch episode data", episode_id=episode_id, error=str(e))
...
async def _prefetch_series_episodes(series_id: int):
    ...
    try:
        ...
        logger.debug("Series episodes prefetched", series_id=series_id, pages=page+1)
    except Exception as e:
        logger.warning("Failed to prefetch series episodes", series_id=series_id, error=str(e))
...
async def _prefetch_series_seasons(series_id: int):
    ...
    try:
        ...
        logger.warning("Failed to prefetch series seasons", series_id=series_id, error=str(e))
...
async def _prefetch_series_metadata(series_id: int):
    ...
    try:
        ...
        logger.warning("Failed to prefetch series metadata", series_id=series_id, error=str(e))
...
def _build_series_search_index(db: Session):
    ...
    for series in series_list:
        ...
        cache.set("search_index", f"series_{series.name.lower()}", search_data, 24)
...
def _build_movie_search_index(db: Session):
    ...
    for movie in movies_list:
        ...
        cache.set("search_index", f"movie_{movie.name.lower()}", search_data, 24)
...
def _build_people_search_index(db: Session):
    ...
    for person in people_list:
        ...
        cache.set("search_index", f"person_{person.name.lower()}", search_data, 24)
