...
def full_sync(self):
    ...
    try:
        with get_db_session() as db:
            ...
            self.update_state(state='PROGRESS', meta={'current': 0, 'total': 100, 'status': 'Starting full sync'})
            
            # Sync static data first
            _sync_static_data_sync(db)
            self.update_state(state='PROGRESS', meta={'current': 20, 'total': 100, 'status': 'Static data synced'})
            
            # Sync all series
            _sync_all_series_sync(db)
            self.update_state(state='PROGRESS', meta={'current': 60, 'total': 100, 'status': 'Series synced'})
            
            # Sync movies
            _sync_movies_sync(db)
            self.update_state(state='PROGRESS', meta={'current': 80, 'total': 100, 'status': 'Movies synced'})
            
            # Sync people
            _sync_people_sync(db)
            self.update_state(state='PROGRESS', meta={'current': 100, 'total': 100, 'status': 'Full sync completed'})
            
            logger.info("Full database sync completed successfully")
            return {"status": "completed", "message": "Full sync completed successfully"}
...
def incremental_sync(self):
    ...
    try:
        with get_db_session() as db:
            self.update_state(state='PROGRESS', meta={'current': 0, 'total': 100, 'status': 'Fetching updates'})
            
            # Get last sync time
            last_sync = _get_last_sync_time(db)
            
            # Fetch updates from TVDB (this would use the /updates endpoint)
            # Note: The Python library doesn't show this endpoint, would need to implement
            ...
            for item in updated_items:
                ...
                self.update_state(
                    state='PROGRESS',
                    meta={'current': progress, 'total': 100, 'status': f'Processing updates: {processed}/{total_items}'}
                )
            
            # Update last sync time
            _update_last_sync_time(db)
            
            logger.info("Incremental sync completed", items_processed=processed)
            return {"status": "completed", "message": f"Processed {processed} updates"}
...
def sync_static_data(self):
    ...
    try:
        with get_db_session() as db:
            ...
            return {"status": "completed", "message": "Static data synced successfully"}
...
def sync_series_detailed(self, series_id: int):
    ...
    try:
        with get_db_session() as db:
            ...
            series_data = asyncio.run(tvdb_client.get_series_extended(series_id, use_cache=False))
            if series_data:
            ...
            page = 0
            while True:
                episodes_data = asyncio.run(tvdb_client.get_series_episodes(series_id, page, use_cache=False))
        ...
        logger.error("Detailed series sync failed", series_id=series_id, error=str(e))
        db.rollback()
        raise
...
def _sync_all_series_sync(db: Session):
    ...
    while True:
        series_data = asyncio.run(tvdb_client.get_all_series(page, use_cache=False))
        ...
        if page % 10 == 0:
            db.commit()
            logger.debug("Committed series batch", page=page, total=total_series)
...
def _update_or_create_episode(db: Session, episode_data: Dict[str, Any], series_id: int):
    ...
    logger.debug("Episode updated/created", tvdb_id=tvdb_id, series_id=series_id)
...
def _process_update_item(db: Session, item: Dict[str, Any]):
    ...
    if entity_type == 'series':
        series_data = asyncio.run(tvdb_client.get_series_extended(entity_id, use_cache=False))
        if series_data:
            _update_or_create_series(db, series_data)
        ...
        movie_data = asyncio.run(tvdb_client.get_movie_extended(entity_id, use_cache=False))
        if movie_data:
