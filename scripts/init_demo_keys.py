...
def init_demo_keys():
    ...
    try:
        ...
        existing_demo1 = db.query(ApiKey).filter(ApiKey.key == "demo-key-1").first()
        existing_demo2 = db.query(ApiKey).filter(ApiKey.key == "demo-key-2").first()
        ...
        demo_keys = [
            ...
            },
            {
                ...
                "description": "Administrative key for API key management (CHANGE IN PRODUCTION)",
                "rate_limit": 1000,
                "active": True,
                "created_by": "system"
            }
        ...
        for key_data in demo_keys:
            ...
            existing = db.query(ApiKey).filter(ApiKey.key == key_data["key"]).first()
            if not existing:
                api_key = ApiKey(**key_data)
                db.add(api_key)
                logger.info("Created demo API key", name=key_data["name"], key_preview=f"...{key_data['key'][-4:]}")
